...
class AugmentMelSTFT(nn.Module):
    def __init__(self, n_mels=128, sr=32000, win_length=800, hopsize=320, n_fft=1024, freqm=48, timem=192,
        ...
        if fmax is None:
            ...
            print(f"=> FMAX is None setting to {fmax} ")
        self.fmax = fmax
    ...
    def forward(self, x):
        ...
        x = torch.view_as_real(torch.stft(x, self.n_fft, hop_length=self.hopsize, win_length=self.win_length, center=True, normalized=False, window=self.window, return_complex=True))
...
class MelTransform(nn.Module):
    def __init__(self):
        ...
        self.mel = AugmentMelSTFT(**MEL_CONFIG)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        old_shape = x.size()
        x = x.reshape(-1, old_shape[2])
        x = self.mel(x)
        x = x.reshape(old_shape[0], old_shape[1], x.size(1), x.size(2))
        return x